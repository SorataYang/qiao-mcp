...
def test_list_api_filters_by_pattern(gateway_provider):
    ...
    result = fns["list_qtmodel_api"](api_object="mdb", pattern="tendon")
    assert "add_tendon_group(name: str = '')" in result
    assert "update_model" not in result
...
def test_call_api_rejects_wrong_kwargs_before_dispatch(gateway_provider):
    ...
    assert "参数不匹配" in result
    assert "add_tendon_group(name: str = '')" in result, "报错须回显真实签名"
    assert gateway_provider._mdb.calls == [], "校验失败不得下发"


def test_call_api_blocks_dangerous_methods(gateway_provider):
    fns = tool_fns(register_api_gateway_tools, gateway_provider)
    result = fns["call_qtmodel_api"](api_object="mdb", method="initial")
    assert "not callable via the gateway" in result or "逃生舱" in result
    assert gateway_provider._mdb.calls == []


def test_call_api_unknown_method(gateway_provider):
    fns = tool_fns(register_api_gateway_tools, gateway_provider)
    result = fns["call_qtmodel_api"](api_object="mdb", method="no_such_method")
    assert "no method" in result
