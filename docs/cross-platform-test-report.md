...
1. ~~**MCP 工具适配**~~: ✅ **已完成**
   - ✅ 结果查询工具自动添加 "ST:" 前缀 (commit c5eaa53)
   - ✅ 文档更新实际的字段名和结构 (commit bba8ab8, bbd54ee)
   - ✅ 连接诊断增强 localhost vs 127.0.0.1 提示 (commit fac6b87)
   - ✅ 修正 is_available() 反映真实连通性 (commit 3f8d01f)
...
### 准确连通性探测
`is_available()` 真实反映后端状态，避免"软件未启动但报告可用"的误导
